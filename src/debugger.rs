...
impl Debugger {
    pub fn launch(exe: PathBuf) -> Result<Self, DebuggerError> {
        ...
        match f {
            ...
            ForkResult::Child => {
                debug!("Exe was forked, PID is [{}].", std::process::id());
    ...
    pub fn find_entrypoint(&self) -> Result<usize, DebuggerError> {
        use std::fs::File;
    ...
    pub fn set_breakpoint(&mut self, address: u64) -> Result<(), BreakpointError> {
        ...
        debug!("  Original instruction: 0x{:x}", orig_byte);
        debug!("  Breakpoint instruction: 0x{:x}", breakpoint_instruction);
    ...
    fn handle_child_status(&mut self, status: WaitStatus) -> Result<(), DebuggerError> {
        debug!("Received signal from child.");
        match status {
